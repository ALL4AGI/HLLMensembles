...
from prompt_parallel import prompt_template


def get_prompts(args):
    if args.dataset == 'gsm8k':
        first_prompt = prompt_template['first_prompt_number']
        other_prompt = prompt_template['other_prompt_number']
    elif args.dataset == 'math':
        first_prompt = prompt_template['first_prompt_math']
        other_prompt = prompt_template['other_prompt_math']
    elif args.dataset in ("mmlu_chemistry", "mmlu_elec", "mmlu_ml", "mmlu_physics"):
        first_prompt = prompt_template['first_prompt_mmlu']
        other_prompt = prompt_template['other_prompt_mmlu']
    elif args.dataset == 'commonsensqa':
        first_prompt = prompt_template['first_prompt_cs']
        other_prompt = prompt_template['other_prompt_cs']
    elif args.dataset == 'strategyqa':
        first_prompt = prompt_template['first_prompt_stQA']
        other_prompt = prompt_template['other_prompt_stQA']
    else:
        raise ValueError(f"Unknown dataset: {args.dataset}")

    return first_prompt, other_prompt


def expert_a(question, history, iteration):
    args = parse_arguments()
    first_prompt, other_prompt = get_prompts(args)

    if iteration == 1:
        prompt = f"""question:{question}\n\nYou are Expert A. {first_prompt}"""
...
def expert_b(question, history, iteration):
    ...
    if iteration == 1:
        prompt = f"""question:{question}\n\nYou are Expert B. {first_prompt}"""
    else:
        prompt = f"""You are Expert B. {other_prompt}"""
...
def interactive(question, selected_experts=None, max_iterations=7):
    ...
    while iteration < max_iterations:
        ...
        answer_a = extract_answer(args, expert_a_solution)
        ...
        if args.dataset == 'math':
            answers = rebuild_answers(answers)
        else:
            answers = answers
        # Check for consensus among the experts
        consistent_answer = None
        unique_answers = set(answer for answer in answers if answer is not None)

        if len(unique_answers) == 1:
            # All experts agree on one answer
            consistent_answer = unique_answers.pop()
        elif len(unique_answers) > 1:
            # Check if there is a majority answer (if at least two experts agree)
            answer_counts = {answer: answers.count(answer) for answer in unique_answers}
            majority_answer = max(answer_counts, key=answer_counts.get)

            if answer_counts[majority_answer] > 1:
                consistent_answer = majority_answer

        if consistent_answer:
            pred = consistent_answer
            break
...
def parse_arguments():
    ...
    parser.add_argument(
        "--output_dir", type=str, default="experiment\Parallel", help="output directory"
    )
    parser.add_argument(
